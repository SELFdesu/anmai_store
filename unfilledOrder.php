<?php
    ...
    $sql="SELECT tb2.productname,tb2.photo,tb2.price,tb2.id,tb1.num,tb1.allprice,tb1.time FROM product_order tb1 INNER JOIN product_show tb2 ON tb1.productid=tb2.id and tb1.userid={$userid} and tb1.send='0' ORDER BY tb1.orderid desc LIMIT {$star_num},{$onePageNum};";
    $result=mysqli_query($link,$sql);
                    ...
                    <?php }else{ ?>
                        <?php for($i=0;$i<count($rows);$i++){?>
                            ...
                            <a class="myorder_content_row_a" href="./product.php?productId=<?php echo $rows[$i]['id']  ?>">
                                ...
                                <div class="content_row_time" style="width: 30%;"><span><?php echo $rows[$i]['time'] ?></span></div>
                                <div class="clear"></div>
                            </a>
                            </div>
                        <?php }?>
