<?php
	...
	$sql="select count(id) from product_comment";
	if(isset($_GET['btn'])){
		$sql="select count(tb1.id) from product_comment tb1 inner join users tb2 on tb1.userid=tb2.id and tb2.username='{$_GET['srch-term']}' ";
	}
	...
	if(!!$proNum[0]){
		...
		$sql="select tb1.*,tb2.username from product_comment tb1 inner join users tb2 on tb1.userid=tb2.id order by tb1.id desc limit {$star_num},{$onePageNum};";
		if(isset($_GET['btn'])){
			$sql="select tb1.*,tb2.username from product_comment tb1 inner join users tb2 on tb1.userid=tb2.id and tb2.username='{$_GET['srch-term']}' order by tb1.id desc limit {$star_num},{$onePageNum};";
		}
		if(isset($_GET['btn']) && $_GET['srch-term']==''){
			$sql="select tb1.*,tb2.username from product_comment tb1 inner join users tb2 on tb1.userid=tb2.id order by tb1.id desc limit {$star_num},{$onePageNum};";
		}
...
<body>
	...
	<div class="templatemo-flex-row">
		<div class="templatemo-sidebar">
			...
			<nav class="templatemo-left-nav">
				<ul>
					...
					<li><a href="#" class="active"><i class="fa fa-bar-chart fa-fw"></i>评论管理</a></li>
		...
		<div class="templatemo-content col-1 light-gray-bg">
			...
			<form  method="post">
			<input type="submit" value="删除" formaction="./dispose/del_checked_com.php" class="btn-danger" style="margin-left: 10px;">
				<div class="templatemo-content-widget no-padding">
						...
						<table class="table table-striped table-bordered templatemo-user-table">
							<thead>
								<tr>
									...
									<td><a href="" class="white-text templatemo-sort-by">评论id <span class="caret"></span></a></td>
							...
							<tbody>
								...
								<tr>
									...
									<td><a href="./dispose/delcomment_dispose.php?commentid=<?php echo $value['id'] ?>&orderid=<?php echo $value['orderid'] ?>" onclick="return confirm('是否确认删除?')" class="templatemo-edit-btn">删除</a></td>
								</tr>
								<?php } ?>
					...
					<?php }else{ ?>
							<div style="text-align: center;line-height:100px;font-size:30px">暂无评论发布！</div>
					<?php } ?>
					</div>
				</div>
