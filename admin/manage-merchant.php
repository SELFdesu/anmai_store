...
if (isset($_GET['btn'])) {
	$sql = "select count(id) from merchant_users where username='{$_GET['srch-term']}'";
}
...
if (!!$proNum[0]) {
	...
	$sql = "select * from merchant_users order by id desc limit {$star_num},{$onePageNum};";
	if (isset($_GET['btn'])) {
		$sql = "select * from merchant_users where username='{$_GET['srch-term']}' order by id desc limit {$star_num},{$onePageNum};";
	}
	if (isset($_GET['btn']) && $_GET['srch-term'] == '') {
		$sql = "select * from merchant_users order by id desc limit {$star_num},{$onePageNum};";
	}
	...
	<div class="templatemo-flex-row">
		<div class="templatemo-sidebar">
			...
			<nav class="templatemo-left-nav">
				<ul>
					...
					<li><a href="#" class="active"><i class="fa fa-user fa-fw"></i>商家管理</a></li>
		...
		<div class="templatemo-content col-1 light-gray-bg">
			...
			<div class="templatemo-content-container">
				<form method="post">
					<input type="submit" value="删除" formaction="./dispose/del_checked_merchant.php" class="btn-danger" style="margin-left: 10px;">
									...
									<tbody>
										<?php foreach ($rows as $value) { ?>
											<tr>
												...
												<td><a href="./dispose/modify_merchantactive_dispose.php?userid=<?php echo $value['id'] ?>&active=<?php echo $value['active'] ?>" class="templatemo-edit-btn"><?php echo $value['active'] ? '冻结' : '恢复' ?></a></td>
												<td><a href="modify_merchant.php?userid=<?php echo $value['id'] ?>" class="templatemo-edit-btn">修改</a></td>
												<td><a href="./dispose/del_merchant_dispose.php?userid=<?php echo $value['id'] ?>" class="templatemo-edit-btn">删除</a></td>
											</tr>
										<?php } ?>
